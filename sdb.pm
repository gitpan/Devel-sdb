...
C<Devel::sdb> is an enhanced version of Perl's (v5.8.8) powerful interactive debugger. The GUI version C<ptkdb> does a very good job already. However, smart debugger takes different approach to problem solving and focuses on non GUI situations. Some of the features added to this version are data rendering, flexible breakpoints, zoompoints, custom mode for debugging fork process and syntax highlighting.
...
=head1 Want more insight to the data?

Ans. B<Data Rendering>
...
=head1 Want more insight to the code?

Ans. B<Code window/markers>
...
=head1 Do you want to breakpoints using regex?

Ans. B<Flexible breakpoints> (SDB_BPTS)
...
=head1 Getting bored with breakpoints? 
...
=head1 Do you want to set your own boundaries for debugging ?
...
=head1 Want more choices in debugging forked process ? 

Ans. B<Custom forktty mode>
...
=head1 So, you want your debugger look pretty?

Ans. B<Syntax Highlight>
...
=head1 Don't want to do everything inside the debugger? 

Ans. B<Debug preferences> (SDB_PREF)
...
=head1  FAQ - Debugger Unleashed

=head1 Lets talk problems...  
...
=head2 then, how do we solve the problem of understanding others' code? 
...
=head1 Lets talk solutions...

=head2 Smart debugger? 
...
=head2 What are smart debugger features?
...
=head1 Smart Debugger, Not a Magic Debugger?
...
# SDB START
# build get_fork_TTY for custom mode
if($$config{'forktty'}{'mode'} == 1) {
    *get_fork_TTY = \&custom_get_fork_TTY;
} 
# SDB END
if (
...
} elsif(not defined &get_fork_TTY) {
    *get_fork_TTY = \&single_get_fork_TTY;      
}
...
=cut
    ...
    if($$config{'syntax'}{mode} == 1) {
         open(LINEINFO, ">$syntfile");
         $LINEINFO = \*LINEINFO;
    } else {
         $LINEINFO = \*DB::OUT unless defined $LINEINFO;
    }
...
=cut
    ...
    unless ($runnonstop) {
        ...
        if ( $term_pid eq '-1' ) {
            ...
            print $OUT
"\nEnter h or `h h' for help, or `perldoc Devel::sdb' for more help.\n\n";
...
# SDB START
# build the context for eval
        my($bname) = $filename;
        $bname =~ s[.*/][]gi;
        local(@saved) = ($@, $!, $^E, $,, $/, $\, $^W);
        local($package, $filename, $line) = caller;
        local($_package, $_filename, $_line, $sub) = caller 1;
        local $usercontext = '($@, $!, $^E, $,, $/, $\\, $^W) = @saved;' . "package $package;";
        local $code = \@{"::_<$filename";};
        local($lname, $lline) = ();
        $sub = $sub || '.';
        return if $$config{'nodebug_onexit'} == 1;

# skip logic for boundary
        unless ($skipfiles =~ /^\.$/) {
            return if $filename =~ /$skipfiles/;
        }
        unless ($skippacks =~ /^\.$/) {
            return if $package =~ /$skippacks/;
        }
        unless ($skipsubs =~ /^\.$/) {
            return if $sub =~ /$skipsubs/;
        }
        unless ($skiplines =~ /^\.$/) {
            return if $$code[$line] =~ /$skiplines/;
        }

# show logic for boundary
        if (grep /$filepat/, $filename) {
         if (grep /$packpat/, $package) {
            if (grep /$subpat/, $sub) {
                $show = 1;
# zoom points logic 
                if ($$config{'code'}{'zoom'}) {
                    if ($$config{'code'}{'zpat'}) {
                        $linepat = $$config{'code'}{'zpat'};
                        if (grep /$linepat/, $$code[$line]) {
                            $show = 1;
                            $$config{'code'}{'zoom'} = 0;
                        }
                        elsif ($$config{'code'}{'zcheck_pre'} or $$config{'code'}{'zcheck_post'}) {
                            $pre = $$config{'code'}{'zcheck_pre'} || 0;
                            $post = $$config{'code'}{'zcheck_post'} || 0;
                            $start = $line - $pre;
                            $end = $line + $post;
                            $start = $start < 0 ? 0 : $start;
                            foreach $lno ($start .. $end) {
                                if (grep /$linepat/, $$code[$lno]) {
                                    $show = 1;
                                    $$config{'code'}{'zoom'} = 0;
                                    last;
                                }
                        ...
                        do {
                            do {
                                do {
                                    ...
                                    print $LINEINFO "\n\n\n############################## SMART DEBUGGER ##############################\n";
# check for data render
                                    if ($$config{'data'}{'render'} == 1) {
                                        print $LINEINFO "-----( DATA )------\n";
                                        if ($$config{'syntax'}{'mode'} == 1) {
                                            close $LINEINFO;
                                        }
...
# greedy variable
                                        ...
                                        (?:[^{])?                   # $@%
                                        (?:[%\@\$])?                # $@%
                                        (?:[[:alnum:]_:]*\s*)?      # variable name
                                        (?: $block )?               # [], {}
                                        /x;


                                        $lastline =~ s/;\s*\#.*//gi;
                                        @vars = $lastline =~ /($pattern)/g; 
                                        ...
                                        foreach $var (sort @vars) {
                                            ...
                                            next if $var =~ /^[\$\@%\\]$/;
                                            $varname = $var;
                                            if ($$config{data}{uniq} == 1) {
                                                next if grep /^\Q$varname\E$/, keys %mhash;
                                            }
                                            next if grep /^\Q$varname\E$/i, @{$$config{'data'}{'skip'};};
                                            $var2 = quotemeta $var;
# data dumper render
                                            if ($$config{'data'}{'mode'} =~ /^1/) {
                                                if ($$config{'data'}{'paddata'}) {
                                                    $print = q[use Data::Dumper;$Data::Dumper::Pad="++";$Data::Dumper::Varname='';];
                                                }
                                                else {
                                                    $print = q[use Data::Dumper;$Data::Dumper::Pad="";$Data::Dumper::Varname='';];
                                                }
                                                ($depth) = $$config{'data'}{'mode'} =~ /^.(.)/;
                                                if ($depth > 0 and $depth < 10) {
                                                    ++$depth;
                                                    $print .= ' $Data::Dumper::Maxdepth = ' . "$depth; ";
                                                }
                                                else {
                                                    $print .= ' $Data::Dumper::Maxdepth = 0; ';
                                                }
                                                if ($$config{'syntax'}{'mode'} == 1) {
                                                    $print .= qq[open LINEINFO, ">>$syntfile";\$LINEINFO = \\*LINEINFO; ];
                                                }
                                                elsif ($$config{'data'}{'stdout'} == 0) {
                                                    $print .= qq[\$LINEINFO = \\*DB::OUT; ];
                                                }
                                                else {
                                                    $print .= '$LINEINFO = "STDOUT";';
                                                }
                                                if ($$config{'data'}{'paddata'}) {
                                                    $print .= qq[print \$LINEINFO "++ $var2 is ";];
                                                }
                                                else {
                                                    $print .= qq[print \$LINEINFO "$var2 is ";];
                                                    if ($$config{data}{subexp} == 1) {
                                                    $var2 = $var;
                                                    $org = $_;
                                                    $_ = $var2;
                                                          while ($_ =~ m/([\$\@\%]\s*)(?=[\$\@\%][[:alnum:]])/gc) {
                                                           $pos = pos();
                                                           $post = $';
                                                           ($nref) = $post =~ /($pattern)/s;
                                                           $post = $';
                                                ...
                                                if ($$config{'data'}{'dumper'}) {
                                                    $print .= 'print $LINEINFO "\\n";';
                                                    $print .= '$Dump =  Dumper [' . join(',', $var) . '];' . '$Dump =~ s/\\\\//gi;' . '$Dump =~ s/^.*?\\$1.*?\\n//gmi; $Dump =~ s/^.*?\\];\\n//gmi;';
                                                }
                                                else {
                                                    $print .= qq[\$Dump =  "$var". "\\n";];
                                                }
                                                if ($$config{'data'}{'sqlbind'}) {
                                                    $print .= q[map { $Dump =~ s/\\?/'$_'/; } (@params);] if $varname =~ /statement/;
                                                    $print .= q[map { $Dump =~ s/\\?/'$_'/; } (@binds);] if $varname =~ /execute/;
                                                    $print .= q[map { $Dump =~ s/\\?/'$_'/; } (@binds);] if $varname =~ /query/;
                                                }
                                                $print .= 'print $LINEINFO $Dump;';
                                                $print .= 'close $OUT;';
                                                my $string = '$^W = 0;' . $usercontext . ' ' . $print . 'print $LINEINFO "\\n"; ';
                                                if ($$config{'syntax'}{'mode'} == 1) {
                                                    $string .= 'close $LINEINFO;';
                                                }
                                                eval $string;
                                                $mhash{$varname} = "$varname";
                                            }
# dumpvar render
                                            if ($$config{'data'}{'mode'} =~ /^2/) {
                                                if ($$config{'data'}{'paddata'}) {
                                                    $print = q[ do 'dumpvar.pl'; ];
                                                }
                                                else {
                                                    $print = q[ do 'dumpvar.pl'; ];
                                                }
                                                ...
                                                if ($$config{'data'}{'dumper'}) {
                                                    $print .= qq[print \$LINEINFO "\n";];
                                                    $print .= 'select($LINEINFO);$Dump =  main::dumpValue( [' . "$var" . '],' . " $maxdepth );";
                                                }
                                                else {
                                                    $print .= qq[\$Dump =  "$var". "\n";];
                                                }
...
# caller render
                                    if ($$config{'code'}{'caller'}) {
                                        print $LINEINFO "-----( CALLER )------\n";
                                        @subs = ();
                                        while (1) {
                                            ($_package, $_filename, $_line, $subroutine, $hasargs, $wantarray, $evaltext, $is_require, $hints, $bitmask) = caller $i;
                                            last if not $subroutine;
                                            unshift @subs, "${_filename}:${_line}:$subroutine";
                                            ++$i;
                                        }
                                        foreach $_ (@subs) {
                                            next if /DB::DB/;
...
# code render
                                    print $LINEINFO "-----( CODE )------\n";
                                    foreach $l ($line - $$config{'code'}{'prelines'} .. $line + $$config{'code'}{'postlines'}) {
                                        if ($l < 1) {
                                            print $LINEINFO "\n" if $$config{'static'};
                                            next;
                                        }
                                        ...
                                        unless ($$config{'code'}{'comments'}) {
                                            $zpat = $$config{'zoom'}{'zpat'};
                                            if ($dbline[$l] =~ /^\s*\#/) {
                                                unless ($zpat and $dbline[$l] =~ /$zpat/) {
                                                    print $LINEINFO "\n" if $$config{'static'};
                                                    next;
                                                }
                                            }
                                        }
                                        if ($$config{'code'}{'padcode'} == 1) {
                                            $position = "${bname}:${sub}:$l";
                                        }
                                        else {
                                            $position = '';
                                        }
                                        $position .= '=>' if $l == $shash{$sub};
                                        $position .= '>>' if $l == $line;
                                        $position .= "\t$dbline[$l]";
                                        print_lineinfo($position);
                                    }
                                    ...
                                    if ($$config{'syntax'}{'mode'} == 1) {
                                        close ($LINEINFO);
                                        $syntdir = "$$config{'syntax'}{'tmpdir'}/";
                                        $syntfile =~ s[//][/]gi;
                                        die "can't open $syntdir" unless -d $syntdir;
                                        $syntfile = "$$config{'syntax'}{'tmpdir'}/$$-syn-temp";
                                        if (-s $syntfile > 1) {
                                            $command = $$config{'syntax'}{'command'};
                                            $command =~ s/%f/$syntfile/gi;
                                            $command =~ s[//][/]gi;
                                            $string = `$command`;
                                            $LINEINFO = \*DB::OUT;
                                            print $LINEINFO $string;
                                            open LINEINFO, ">$syntfile";
                                            $LINEINFO = \*LINEINFO;
                                        }
                                    }
...
# SDB START
    @b = @{$$config{'breakpt'}{'b'};};
    @b_load = @{$$config{'breakpt'}{'b_load'};};
    @b_compile = @{$$config{'breakpt'}{'b_compile'};};
    @b_postpone = @{$$config{'breakpt'}{'b_postpone'};};
    if ($ENV{'SDB_BPTS'}) {
        @b = split(/\s+/, $ENV{'SDB_BPTS'});
    }
    if (@b) {
        foreach $bpoint (@b) {
            $bpoint =~ s/(^\s*|\s*$)//gi;
            chomp $bpoint;
            next unless $bpoint;
            @fnames = grep(/.*$bpoint/, sort(keys %sub));
            foreach $fullname (@fnames) {
                &cmd_b('b', "$fullname");
            }
        }
    }
    if (@b_load) {
        foreach $bpoint (@b_load) {
            $bpoint =~ s/(^\s*|\s*$)//gi;
            chomp $bpoint;
            next unless $bpoint;
            @fnames = grep(/.*$bpoint/, sort(keys %INC));
            foreach $fullname (@fnames) {
                &cmd_b_load($fullname);
            }
        }
    ...
    if (@b_compile) {
        foreach $bpoint (@b_compile) {
            $bpoint =~ s/(^\s*|\s*$)//gi;
            chomp $bpoint;
            next unless $bpoint;
            @fnames = grep(/.*$bpoint/, sort(keys %sub));
            foreach $fullname (@fnames) {
                &cmd_b('b compile', "$fullname");
            }
        }
    }
    if (@b_postpone) {
        foreach $bpoint (@b_postpone) {
            $bpoint =~ s/(^\s*|\s*$)//gi;
            chomp $bpoint;
            next unless $bpoint;
            @fnames = grep(/.*$bpoint/, sort(keys %sub));
            foreach $fullname (@fnames) {
                &cmd_b('b postpone', "$fullname");
            }
        }
    }
      ...
      CMD:
        ...
        $cmd ||= $$config{'enterkey'} || 's';
        if ($cmd =~ /^list b(reakpoint)?/) {
            if (@b) {
                print $OUT "listing recent -- b (subs)\n";
                foreach $bpoint (@b) {
                    chomp $bpoint;
                    next unless($bpoint);
                    @fnames = grep(/.*$bpoint/, sort(keys %sub));
                    print $OUT Dumper([@fnames]);
                }
            }
            if (defined @b_load) {
                print $OUT "listing recent -- b load\n";
                foreach $bpoint (@b_load) {
                    chomp $bpoint;
                    next unless($bpoint);
                    @fnames = grep(/.*$bpoint/, sort(keys %INC));
            ...
            if (@b_compile) {
                print $OUT "listing recent -- b compile\n";
                foreach $bpoint (@b_compile) {
                    chomp $bpoint;
                    next unless($bpoint);
                    @fnames = grep(/.*$bpoint/, sort(keys %sub));
            ...
            if (@b_postpone) {
                print $OUT "listing recent -- b postpone\n";
                foreach $bpoint (@b_postpone) {
                    chomp $bpoint;
                    next unless($bpoint);
                    @fnames = grep(/.*$bpoint/, sort(keys %sub));
        ...
        elsif ($cmd =~ /^list\s+p(references)?/) {
            print "listing -- preferences\n";
            $tmpconfig = {
            'code'   =>  {
              'window'   => $$config{code}{window},
              'zwindow'  => $$config{code}{zwindow},
              'caller'   => $$config{code}{caller},
              'comments' => $$config{code}{comments},
              'warnings' => $$config{code}{warnings},
              'padcode'  => $$config{code}{padcode},
              'quickexit'=> $$config{code}{quickexit},
             },
            'data'   =>  {
              'render'    => $$config{data}{render},
              'subexp'   => $$config{data}{subexp},
              'mode'     => $$config{data}{mode},
              'skip'     => $$config{data}{skip},
             },
            'breakpt'    =>  $$config{breakpt},
            'boundary'   =>  $$config{boundary},
            'forktty'    =>  $$config{forktty},
            'syntax'     =>  $$config{syntax},
            };
            print Dumper $tmpconfig;
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+q(uick)?/) {
            $$config{'code'}{'quickexit'} = 1;
            print $OUT "quick exit-- turned on \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+noq(uick)?/) {
            $$config{'code'}{'quickexit'} = 0;
            print $OUT "quick exit-- turned off \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+dr(ender)?/) {
            $$config{'data'}{'render'} = 1;
            print $OUT "data render -- turned on \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+nodr(ender)?/) {
            $$config{'data'}{'render'} = 0;
            print $OUT "data render -- turned off \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+dm(ode)?\s+(\d+)/) {
            $$config{'data'}{'mode'} = $2;
            print $OUT "datamode -- set to $2\n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+cw(size)?\s+(.*)/) {
            $window = $2;
            ($pre, $post) = split(/,/, $window, 3);
            if ($pre < 0) {
                $pre =~ s/\-//gi;
                $$config{'code'}{'prelines'} = $pre;
            }
            else {
                $$config{'code'}{'postlines'} = $pre;
            }
            if ($post < 0) {
                $post =~ s/\-//gi;
                $$config{'code'}{'prelines'} = $post;
            }
            else {
                $$config{'code'}{'postlines'} = $post;
            }
            print $OUT "window size -- set to -$pre,$post \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+c(aller)?/) {
            $$config{'code'}{'caller'} = 1;
            print $OUT "caller -- turned on \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+noc(aller)?/) {
            $$config{'code'}{'caller'} = 0;
            print $OUT "caller -- turned off \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+p(ad)?/) {
            $$config{'code'}{'padcode'} = 1;
            print $OUT "padcode -- turned on \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+nop(ad)?/) {
            $$config{'code'}{'padcode'} = 0;
            print $OUT "padcode -- turned off \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+zw(size)?\s+(.*)/) {
            $zcheck = $2;
            ($pre, $post) = split(/,/, $zcheck, 3);
            ($dpre, $dpost) = split(/,/, $$config{'code'}{'zwindow'}, 3);
            $pre ||= 0;
            $post ||= 0;
            if ($pre <= 0) {
                $pre =~ s/\-//gi;
                $$config{'code'}{'zcheck_pre'} = $pre;
            }
            else {
                $$config{'code'}{'zcheck_post'} = $pre;
            }
            if ($post < 0) {
                $post =~ s/\-//gi;
                $$config{'code'}{'zcheck_pre'} = $post;
            }
            else {
                $$config{'code'}{'zcheck_post'} = $post;
            }
            $pre = '-' . $$config{'code'}{'zcheck_pre'};
            print $OUT "zoom window -- set to $pre,$post \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ m[^z\s*/(.*)]) {
            $pattern = $1;
            $$config{'code'}{'zpat'} = "$pattern";
            $pre = '-' . $$config{'code'}{'zcheck_pre'};
            $post = $$config{'code'}{'zcheck_post'};
            print $OUT "($pre,$post) zooming to pattern /$pattern \n";
            $$config{'code'}{'zoom'} = 1;
            $cmd = 's';
        }
        elsif ($cmd =~ /^z/) {
            $$config{'code'}{'zoom'} = 1;
            $pattern = $$config{'code'}{'zpat'};
            $pre = '-' . $$config{'code'}{'zcheck_pre'};
            $post = $$config{'code'}{'zcheck_post'};
            print $OUT "($pre,$post) zooming to pattern /$pattern \n";
            $cmd = 's';
        }
        elsif ($cmd =~ /^set f(ttymode)? (\d+)/) {
            $mode = $2;
            chomp $mode;
            $$config{'forktty'}{'mode'} = $mode;
            print $OUT "fork tty mode -- set to $mode\n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+b(reakpoint)?\s+compile\s+(.*)/) {
            $breakpoint2 = $2;
            @b_compile = split(?\s+?, $breakpoint2, 0);
            foreach $bpoint (@b_compile) {
                $bpoint =~ s/(^\s*|\s*$)//gi;
                chomp $bpoint;
                next unless $bpoint;
                ...
                print $OUT "setting breakpoints -- b compile\n";
                print $OUT Dumper([@fnames]);
            }
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+b(reakpoint)?\s+postpone\s+(.*)/) {
            $breakpoint2 = $2;
            @b_postpone = split(?\s+?, $breakpoint2, 0);
            foreach $bpoint (@b_postpone) {
                $bpoint =~ s/(^\s*|\s*$)//gi;
                chomp $bpoint;
                next unless $bpoint;
                ...
                print $OUT "setting breakpoints -- b postpone \n";
                print $OUT Dumper([@fnames]);
            }
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+b(reakpoint)?\s+load\s+(.*)/) {
            $breakpoint2 = $2;
            @b_load = split(?\s+?, $breakpoint2, 0);
            foreach $bpoint (@b_load) {
                $bpoint =~ s/(^\s*|\s*$)//gi;
                chomp $bpoint;
                next unless $bpoint;
                ...
                print $OUT "setting breakpoints -- b load \n";
                print $OUT Dumper([@fnames]);
            }
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+b(reakpoint)?\s+(.*)/) {
            $breakpoint2 = $2;
            @b = split(?\s+?, $breakpoint2, 0);
            foreach $bpoint (@b) {
                $bpoint =~ s/(^\s*|\s*$)//gi;
                chomp $bpoint;
                next unless $bpoint;
                ...
                print $OUT "setting breakpoints -- b (subs)\n";
                print $OUT Dumper([@fnames]);
            }
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+j(ump)?/) {
            $$config{'enterkey'} = 'c';
            print $OUT "enterkey -- is now 'c' \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+n(ext)?/) {
            $$config{'enterkey'} = 'n';
            print $OUT "enterkey -- is now 'n' \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+z(oom)?/) {
            $$config{'enterkey'} = 'z';
            print $OUT "enterkey -- is now 'z' \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^set\s+s(ingle)?/) {
            $$config{'enterkey'} = 's';
            print $OUT "enterkey -- is now 's' \n";
            $cmd = 'XYZ';
        }
        elsif ($cmd =~ /^(set|list|z)/) {
...
=cut
                ...
                $cmd =~ /^q$/ && do {
                      $fall_off_end = 1;
                      clean_ENV();
                      if($$config{code}{'quickexit'}) {
                      $signal = 1;
                      $$config{'nodebug_onexit'} = 1;
                      }
                      exit $?;
                };
...
sub xterm_get_fork_TTY {
    ...
    chomp $tty;
    $pidprompt = '';
    $$config{'data'}{'datatty'} = $tty;
    $$config{'forked'} = 1;
    return $tty;
}
sub screen_get_fork_TTY {
    return '' unless open my $SCREEN, "screen -t 'Child $$' |";
    my $tty = <XT>;
    chomp $tty;
    $DB::fork_TTY = $tty;
    $$config{'data'}{'datatty'} = $DB::fork_TTY;
    $$config{'forked'} = 1;
    return $tty;
}
sub custom_get_fork_TTY {
    my $tty = `cat $countfile` || 0;
    $DB::fork_TTY = $fttys[$tty];
    ++$tty;
    `echo $tty > $countfile`;
    $$config{'data'}{'datatty'} = $DB::fork_TTY;
    chomp $DB::fork_TTY;
    ...
    if (
        ...
        and (
            ( $kpid = CORE::system 4, $^X, '-we',
                <<'ES', @args ) >= 0    # P_SESSION
sub db_END {sleep 5 unless $loaded}
...
B<set dr>[ender]                        set datarender        
B<set nodr>[ender]                      unset datarender
B<set dm>[ode] 1[1-9]                   datadumper,depth  
B<set dm>[ode] 2[1-9]                   dumpvar,depth
B<set c>[aller]                         set caller segment      
B<set noc>[aller]                       unset caller segment
B<set p>[ad]                            set padding code        
B<set nop>[ad]                          unset padding code
B<set q>[uick]                          set quick exit (skip cleanup routines)         
B<set noq>[uick]                        unset quick exit
B<set j>[ump]                           set enter key map to 'c' 
B<set n>[ext]                           set enter key map to 'n'
B<set s>[ingle]                         set enter key map to 's' 
B<set z>[oom]                           set enter key map to 'z'
B<set b>[reakpoint] expr                set sdb style - b (subs)
B<set b>[reakpoint] compile expr        set sdb style - b compile
B<set b>[reakpoint] postpone expr       set sdb style - b postpone
B<set b>[reakpoint] load expr           set sdb style - b load
B<list b>[reakpoint]                    display the recent sdb style breakpoint
B<list p>[references]                   display the preferences
B<set cw>[size]                         -pre,post set code window size
B<set zw>[size]                         -pre,post set zoom window size
B<z> /pattern                           zoom to pattern with zwindow
B<z>                                    zoom next
...
I<SDB Commands:>  
  B<set dr>[ender]  set datarender        B<set nodr>[ender] unset datarender
  B<set dm>[ode] 1[1-9] datadumper,depth  B<set dm>[ode] 2[1-9] dumpvar,depth
  B<set c>[aller] set caller segment      B<set noc>[aller] unset caller segment
  B<set p>[ad]    set padding code        B<set nop>[ad] unset padding code
  B<set q>[uick]  set quick exit          B<set noq>[uick] unset quick exit
  B<set j>[ump]  set enter key map to 'c' B<set n>[ext] set enter key map to 'n'
  B<set s>[ingle]set enter key map to 's' B<set z>[oom] set enter key map to 'z'
  B<set b>[reakpoint] expr                set sdb style breakpoints
  B<set b>[reakpoint] compile expr        set sdb style "b compile"
  B<set b>[reakpoint] postpone expr       set sdb style "b postpone"
  B<set b>[reakpoint] load expr           set sdb style "b load"
  B<list b>[reakpoint]                    display the recent sdb style breakpoint
  ...
  B<z> /pattern                           zoom to pattern with zwindow
  B<z>                                    zoom next
For more help, type B<h> I<cmd_letter>, or run B<perldoc Devel::sdb> for all docs.
END_SUM
...
BEGIN {
    ...
    'forktty' => {
      'mode'     => 0, # 0 = normal, 1 = custom
      'tmpdir'   => "$ENV{HOME}/.sdb/fork/",
     },
    'syntax' =>  {
      'mode'     => 0, # 0 = normal, 1 = highlight
      'tmpdir'   => "$ENV{HOME}/.sdb/syntax/",
      'command'  => 'cat %f',
     },
    };
    ...
    if ($config) {
        $config = {%$defconf, %$config};
        $$config{'code'} = {%{$$defconf{'code'};}, %{$$config{'code'};}};
        $$config{'data'} = {%{$$defconf{'data'};}, %{$$config{'data'};}};
        $$config{'boundary'} = {%{$$defconf{'boundary'};}, %{$$config{'boundary'};}};
        $$config{'boundary'}{'show'} = {%{$$defconf{'boundary'}{'show'};}, %{$$config{'boundary'}{'show'};}};
        $$config{'boundary'}{'skip'} = {%{$$defconf{'boundary'}{'skip'};}, %{$$config{'boundary'}{'skip'};}};
        $$config{'skip'} = {%{$$defconf{'skip'};}, %{$$config{'skip'};}};
        $$config{'forktty'} = {%{$$defconf{'forktty'};}, %{$$config{'forktty'};}};
        $$config{'syntax'} = {%{$$defconf{'syntax'};}, %{$$config{'syntax'};}};
    }
    $config = $defconf unless $config;
    $filepat = join('|', @{$$config{'boundary'}{'show'}{'files'};});
    $packpat = join('|', @{$$config{'boundary'}{'show'}{'packs'};});
    $subpat = join('|', @{$$config{'boundary'}{'show'}{'subs'};});
    $plinepat = join('|', @{$$config{'boundary'}{'plines'};});
    $callpat = join('|', @{$$config{'boundary'}{'callback'};});
    $skipfiles = join('|', @{$$config{'boundary'}{'skip'}{'files'};});
    $skippacks = join('|', @{$$config{'boundary'}{'skip'}{'packs'};});
    $skipsubs = join('|', @{$$config{'boundary'}{'skip'}{'subs'};});
    $skipvars = join('|', @{$$config{'data'}{'skip'};});
    $filepat = $filepat ? "($filepat)" : '.';
    $packpat = $packpat ? "($packpat)" : '.';
    $subpat = $subpat ? "($subpat)" : '.';
    $linepat = $linepat ? "($linepat)" : '.';
    $plinepat = $plinepat ? "($plinepat)" : '.';
    $callpat = $callpat ? "($callpat)" : '.';
    $skipfiles = $skipfiles ? "($skipfiles)" : '.';
    $skippacks = $skippacks ? "($skippacks)" : '.';
    $skipsubs = $skipsubs ? "($skipsubs)" : '.';
    $skipvars = $skipvars ? "($skipvars)" : '.';
    ...
    ($pre, $post) = split(/,/, $$config{'code'}{'zwindow'}, 3);
    $pre ||= 0;
    $post ||= 0;
    if ($pre <= 0) {
        $pre =~ s/\-//gi;
    ...
    $inhibit_exit = $$config{'code'}{'inhibit'};
...
sub END {
    ...
    exit if $ENV{'SDB_TEST'};
    print $OUT ">> END \n";
    unless ($$config{'nodebug_onexit'}) {
        print $OUT "(ctrl-c to exit)\n";
        sleep $$config{'code'}{'endsleep'};
    }
}
